...
contract Staking {
    ...
    function stakeEther(uint numDays) external payable {
        ...
        positions[currentPositionId] = Position(
            ...
            tiers[numDays],
            msg.value,
            calculateInterest(tiers[numbDays], numDays, msg.value),
            true
        );
    ...
    function PositionById(uint positionId) external view returns (Position memory) {
    ...
    function closePosition(uint positionId) external  {
        require(positions[positionId].walletAddress == msg.sender, "Only position creator may modify staking periods");
